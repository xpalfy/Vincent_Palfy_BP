...
{
    $slovakName = filter_input(INPUT_POST, 'slovakName', FILTER_UNSAFE_RAW);
    $englishName = filter_input(INPUT_POST, 'englishName', FILTER_UNSAFE_RAW);
...
function handlePdfUpload()
{
    ...
    if (!empty($_FILES['fileToUpload']['name'])) {
        ...
        if ($uploadOk == 0) {
            $_SESSION['toast'] = array("status" => "error", "message" => "Sorry, your file was not uploaded.");
            return "";
        } else {
...
<head>
    ...
    <title>Vytvorenie novej témy</title>
                ...
                if (lecture) {
                    fetch(`learn/getTopics.php?lecture=${lecture}`)
                        ...
                        .then(data => {
                            ...
                            topicSelect.innerHTML = '<option value="">Vyberte tému</option>';
                            data.forEach(topic => {
                                const option = new Option(topic.slovak_name, topic.id);
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarResponsive">
            <ul class="navbar-nav ml-auto">
                ...
                <li class="nav-item"><a class="nav-link" href="/html/teacher/english/learn.php">Anglická verzia</a></li>
...
<div class="container">
    ...
    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post" enctype="multipart/form-data"
        ...
        <div id="pdfUpload" style="display: none;">
            <div class="form-group">
                <label for="fileToUpload">Nahrať PDF:</label>
                <div class="form-group custom-file">
                    <input type="file" class="custom-file-input" id="fileToUpload" name="fileToUpload" accept=".pdf">
                    <label class="custom-file-label" for="fileToUpload">Vyberte si súbor</label>
                </div>
            </div>
        </div>
        <div class="form-group">
            <div class="row">
                ...
                <div class="col-md-6 mt-2 mt-md-0">
                    <button type="button" class="btn btn-primary btn-block" onclick="togglePDFUpload()">Pridať nový
                        súbor PDF
                    </button>
                </div>
            </div>
    ...
    <h2>Odstrániť tému</h2>
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
        <input type="hidden" name="csrf_token" value="<?php echo generateCsrfToken(); ?>">
        <div class="form-group">
            <label for="lectureSelectForTopics">Vybrať prednášku:</label>
            <select class="form-control" id="lectureSelectForTopics" name="lecture">
                <option value="">Vybrať prednášku</option>
                <?php
                $result = $masterDB->query("SELECT name FROM learn");
                while ($row = $result->fetch_assoc()) {
    ...
    <div id="deleteTopicSection" style="display: none;">
        <form action="" id="deleteTopicForm" name="deleteTopicForm" method="post">
            <input type="hidden" name="csrf_token" value="<?php echo generateCsrfToken(); ?>">
            <div class="form-group">
                <label for="topicID">Vyberte tému</label>
...
<script>
    ...
    function checkFormB(e) {
        ...
        if (valid) {
            ...
            toastr.error("Prosím, vyplňte všetky polia správne.");
        }
    }
</script>
<?php $masterDB->close(); ?>
